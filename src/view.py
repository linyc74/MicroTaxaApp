...
class View(QWidget):
    ...
    def __show_mode(self):
        for edit in self.edit_dict.values():
            if edit.key in self.mode.MICROTAXA_KEYS + self.mode.SSH_KEYS:
                edit.qlabel.show()
                edit.qedit.show()
            else:
                edit.qlabel.hide()
    ...
    def get_key_values(self) -> Dict[str, Union[str, bool]]:
        return self.__get_key_values(keys=self.mode.SSH_KEYS + self.mode.MICROTAXA_KEYS)

    def get_ssh_key_values(self) -> Dict[str, Union[str, bool]]:
        return self.__get_key_values(keys=self.mode.SSH_KEYS)

    def get_microtaxa_key_values(self) -> Dict[str, Union[str, bool]]:
        return self.__get_key_values(keys=self.mode.MICROTAXA_KEYS)
