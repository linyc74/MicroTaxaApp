...
class ActionSubmit(Action):
    ...
    microtaxa_key_values: Dict[str, str]
    con: Connection
    microtaxa_cmd: str
    submit_cmd: str
    ...
    def get_key_values(self):
        ...
        self.microtaxa_key_values = self.view.get_microtaxa_key_values()

    def set_microtaxa_cmd(self):
        microtaxa = self.ssh_key_values['MicroTaxa']
        outdir = self.microtaxa_key_values['outdir']

        args = [f'python {microtaxa}']
        for key, val in self.microtaxa_key_values.items():
            if type(val) is bool:
                if val is True:
                    args.append(f'--{key}')
        ...
        self.microtaxa_cmd = ' '.join(args)

        if '"' in self.microtaxa_cmd:
            print('Warning: double quotes in the MicroTaxa command will be replaced by single quotes', flush=True)
            # self.microtaxa_cmd will be wrapped in double quotes in self.submit_cmd
            # so double quotes needs to be avoided
            self.microtaxa_cmd = self.microtaxa_cmd.replace('"', '\'')

    def set_submit_cmd(self):
        outdir = self.microtaxa_key_values['outdir']
        job_name = basename(outdir).replace(' ', '_')
        sample_sheet = self.microtaxa_key_values['sample-sheet']

        # the environment (.bash_profile) needs to be activated right before the microtaxa_cmd
        script = f'source {self.BASH_PROFILE} && {self.microtaxa_cmd}'
